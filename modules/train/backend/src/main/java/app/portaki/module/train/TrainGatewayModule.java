...
@Component
@PortakiModule("train")
public class TrainGatewayModule {

    private final JdbcTemplate jdbc;
    private final ObjectMapper objectMapper;

    public TrainGatewayModule(JdbcTemplate jdbc, ObjectMapper objectMapper) {
        this.jdbc = jdbc;
        this.objectMapper = objectMapper;
    }

    @PortakiQueryHandler(value = "train.departures", scope = "stay:read")
    public List<Map<String, Object>> listDepartures(Map<String, Object> params, GatewayModuleContext ctx) {
        String stationCode = String.valueOf(ctx.config().getOrDefault("departure_station_code", "")).trim();
        String cacheKey =
                "train|" + ctx.tenantIdInternal() + "|" + UUID.fromString(ctx.propertyId()) + "|" + stationCode;
        return jdbc.query(
                """
                SELECT payload, expires_at
                FROM t_e_module_train_cache
                WHERE cache_key = ?
                """,
    ...
    private List<Map<String, Object>> readCachePayload(java.sql.ResultSet rs) throws java.sql.SQLException {
        ...
        try {
            ...
            throw new IllegalStateException("train_cache_payload_invalid", e);
        }
    }

...
- **Automatique (semver)** : sur un **push `main`** du dépôt **`PortakiApp/portaki-modules`**, le workflow **[`ci.yml`](../.github/workflows/ci.yml)** (**Checks**) exécute `pnpm registry-publish:npm` (`scripts/registry-publish.mjs`) : chaque paquet **`@portaki/module-*`** n’est publié que si la **`version`** locale est **strictement supérieure** à la dernière version publiée sur npmjs (Trusted Publishing OIDC, `id-token: write` sur les jobs de publication).
...
- **Automatique (semver)** : sur un **push `main`** (`PortakiApp/portaki-modules`), le workflow **[`ci.yml`](../.github/workflows/ci.yml)** exécute `pnpm registry-publish:maven` : chaque `modules/*/backend` avec `pom.xml` **release** (pas `-SNAPSHOT`) n’est déployé que si la version du POM est **strictement supérieure** à `<latest>` dans `maven-metadata.xml` sur repo1.maven.org. Nécessite **`OSSRH_*`** + **GPG** ; sinon le job affiche une notice et ne déploie pas.

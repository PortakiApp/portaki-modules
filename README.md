...
Le SDK **`@portaki/module-sdk`** (types, `definePortakiModule`) est publié depuis **[portaki-sdk](https://github.com/PortakiApp/portaki-sdk)** uniquement. La feuille de route architecture plateforme (Wasm, runtime, DB, coûts) est dans le dossier interne **`portaki-internal-docs/`** (workspace **Repositories**, hors dépôt public).
...
- **Checks** : [`.github/workflows/ci.yml`](./.github/workflows/ci.yml) — workflow **Checks** : découverte des backends Java, **trois jobs Node en parallèle** (`assert-no-file-deps`, `validate:modules`, `lint`), matrice `mvn verify` sur les `modules/*/backend`. Sur **push `main`** (`PortakiApp/portaki-modules`) : publication semver **npm** (après uniquement les checks Node + découverte npm) et **Maven** (après checks Node + `mvn verify` + découverte Maven) — voir ci-dessous.

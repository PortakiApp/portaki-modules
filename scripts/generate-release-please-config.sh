#!/usr/bin/env bash
# Discover modules/*/Cargo.toml and regenerate
...
# Discover releasable modules: a crate under modules/*/. Its manifest is written by
# `portaki build` from the code; the version lives in Cargo.toml only.
MODULES_FILE="$(mktemp)"
...
for mod_id, ver in modules:
    ...
    # publishes the version its Cargo.toml carries (the `ci` publish
...
config["pull-request-footer"] = (
    ...
    "- [ ] An SDK bump is NOT in here. release-please attributes commits by path "
    "(`No commits for path: modules/...`), so a root-only bump releases nothing and "
    "the artifacts keep announcing the old SDK -- see the root `Cargo.toml`\n"
